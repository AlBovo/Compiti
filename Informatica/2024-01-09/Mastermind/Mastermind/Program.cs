    ...
    internal class Program
    {
        ...
        static int LeggiTentativi()
        {
            ...
            while (true)
            {
                ...
                ConsoleColor[] colors = new ConsoleColor[] { ConsoleColor.Red, ConsoleColor.Green, ConsoleColor.Blue, ConsoleColor.Magenta, ConsoleColor.Yellow }; // array di colori
                Console.ForegroundColor = colors[rand.Next(0, colors.Length)];  // imposto un colore a caso
                ...
                #region Stampa di un errore se presente
                if (error != "") // stampo un errore se presente
                {
                ...
                #region Stampa del menu
                Console.WriteLine("\nScegli una difficoltà");
                Console.WriteLine("[A] Impossibile (2 tentativi)");
                Console.WriteLine("[B] Difficile   (5 tentativi)");
                Console.WriteLine("[C] Medio       (10 tentativi)");
                Console.WriteLine("[D] Facile      (20 tentativi)");
                Console.Write("\nInserisci la tua scelta: ");
                #endregion

                char input = Console.ReadLine().ToUpper()[0]; // leggo la scelta e la metto uppercase
                ...
                switch (input)
                {
                    ...
                    case 'D': // facile
                        return 20;
                    default:
                        error = "L'input inserito non è valido";
                        break;
                }
                Console.Clear();
            }
